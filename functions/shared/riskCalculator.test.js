...
const { calculateRisk, LEGACY_PHASES } = require('../shared/riskCalculator');

// =============================================================================
// SYNC: Estes testes devem ser idênticos a src/core/riskCalculator.test.js.
...
describe('calculateRisk — flags INCONCLUSIVE / NEGATIVE_PARTIAL', () => {
    ...
    it('NEGATIVE_PARTIAL sozinho → score 18, YELLOW, ATTENTION', () => {
        const r = calculateRisk({ criminalFlag: 'NEGATIVE_PARTIAL' });
        expect(r.riskScore).toBe(18);
        // 18 < 30 → pode ser YELLOW por yellowSignal ou GREEN se não há yellow signals suficientes
        // NEGATIVE_PARTIAL é 1 sinal amarelo; sozinho < 2 → sem boost; score < 30 → GREEN
        expect(r.riskLevel).toBe('GREEN');
        expect(r.suggestedVerdict).toBe('FIT');
    });
...
describe('calculateRisk — limites e consistência', () => {
    it('score não ultrapassa 100', () => {
        // Muitos sinais amarelos + POSITIVE
        const r = calculateRisk({
