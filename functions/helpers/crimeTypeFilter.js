...
const CONSUMER_CIVIL_NOISE_PATTERN = /\b(DIREITO\s+DO\s+CONSUMIDOR|RESPONSABILIDADE\s+CIVIL|INDENIZACAO\s+POR\s+DANO|ACIDENTE\s+DE\s+TRANSITO|CARTAO\s+DE\s+CREDITO|INCLUSAO\s+INDEVIDA|ALUGUEIS|DESPEJO|OBRAS\s+SOCIAIS|PATRIMONIO\s+CULTURAL|NOTA\s+PROMISSORIA|COBRANCA|COMPRA\s+E\s+VENDA|JUIZADO\s+ESPECIAL\s+CIVEL|PROCEDIMENTO\s+COMUM\s+CIVEL|DIREITO\s+CIVIL|DUPLICATA|TITULO\s+EXTRAJUDICIAL|EXECUCAO\s+DE\s+TITULO|OBRIGACOES|ADIANTAMENTO\s+A\s+DEPOSITARIO|REINTEGRACAO\s+DE\s+POSSE|CONDOMINIO|REVISIONAL|EXECUCAO\s+FISCAL|ALIENACAO\s+FIDUCIARIA|BUSCA\s+E\s+APREENSAO|PRISAO\s+CIVIL|PETICAO\s+CIVEL|CUMPRIMENTO\s+PROVISORIO|IPTU|PENSAO\s+ALIMENTICIA|TARIFAS|ALIMENTOS\s+-|DIREITO\s+PROCESSUAL\s+CIVIL|ALIENACAO\s+FIDUCIARIA|PROCEDIMENTO\s+DO\s+JUIZADO\s+ESPECIAL\s+CIVEL)\b/i;

// Palavras-chave que CONFIRMAM que um processo com area=CRIMINAL
// realmente eh criminal (protege contra falsos positivos da API do Escavador2).
// Sem esse indicador, o processo nao eh contado como criminal material.
const CRIMINAL_INDICATOR_PATTERN = /\b(ACAO\s+PENAL|INQUERITO\s+POLICIAL|PROCESSO\s+CRIMINAL|TERMO\s+CIRCUNSTANCIADO|FLAGRANTE|ESTUPRO|ROUBO|FURTO|TRAFICO|DROGAS|HOMICIDIO|VIOLENCIA\s+DOMESTICA|MARIA\s+DA\s+PENHA|LEI\s+ANTITOXICOS|LESAO\s+CORPORAL|AMEACA|CONSTRANGIMENTO|PERSEGUIM|EXECUCAO\s+PENAL|PENA\s+PRIVATIVA|CONTRAVENCOES?\s+PENAIS?|RECEPTACAO|DESACATO|ESTELIONATO|INJURIA|CALUNIA|EXTORSAO|CRIMES?\s+CONTRA|PROCEDIMENTO\s+DO\s+JUIZADO\s+ESPECIAL\s+CRIMINAL|AUTO\s+DE\s+PRISAO|MEDIDAS\s+PROTETIVAS)\b/i;
