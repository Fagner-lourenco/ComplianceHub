...
const HIGH_RISK_CRIMINAL_ROLES = /^(REU|RE\s*$|REU\s+(RE|S)|INDICIAD[OA]|INDICIADO\s+A|INVESTIGAD[OA]|INVESTIGADO\s+A|AUTOR\s*(?:A)?\s+DO\s+FATO|AUTORA\s+DO\s+FATO|AUTOR\s+FATO|CONDENAD[OA]|ACUSAD[OA]|ACUSADO\s+A|AVERIGUAD[OA]|EXECUTADO|EXECTD[OA]|EXECD[OA]|REEDUCANDO|BENEFICIARIO|SUJE?TO|AGENTE|DENUNCIAD[OA]|DENUNCIADO\s+A|NOTICIAD[OA]|AUTUAD[OA]|FLAGRANTEAD[OA]|FLAGRANTEADO\s*\(?A\)?|SENTENCIAD[OA]|EM\s+APURACAO|POLO\s+PASSIVO|PASSIVO|PACIENTE|PROMOVIDO|DEPRECAD[OA]|DEPRECADO\s+A|INFRATOR|CORREU|APELANTE|APELADO|RECORRENTE|RECORRIDO|AGRAVANTE|AGRAVADO)$/i;
...
const VICTIM_ROLES = /^(VITIMA|V\b|VITIMA\s+DO\s+FATO|OFENDIDO|OFENDIDA|OFENDIDO\s+DO\s+FATO|OFENDIDA\s+DO\s+FATO|PREJUDICADO|LESADO|DAMNIFICADO|AGRAVIADO|PREJUDICADA|LESADA|DAMNIFICADA|AGRAVIADA)$/i;
...
// Testemunhas - ignorar
const WITNESS_ROLES = /^(TESTEMUNHA|TESTEMUNHA\s+DO\s+JUIZO|TESTEMUNHA\s+POLO\s+ATIVO|TESTEMUNHA\s+POLO\s+PASSIVO|TESTEMUNHA\s+DE\s+ACUSACAO|TESTEMUNHA\s+PARTE\s+ATIVA|INFORMANTE|INFORMADO|INFORMADA)$/i;
