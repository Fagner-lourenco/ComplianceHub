...
const HIGH_RISK_CRIMINAL_ROLES = /^(REU|RE\s*$|REU\s+(RE|S|\d+|A)|INDICIAD[OA](?:\s+A)?|INVESTIGAD[OA](?:\s+A)?|AUTOR\s*(?:A)?\s+DO\s+FATO|AUTOR\s+FATO|CONDENAD[OA]|ACUSAD[OA](?:\s+A)?|AVERIGUAD[OA]|AVERIGUADA|EXECUTADO|EXECTD[OA]|EXECD[OA]|EXECDO|EXECDA|REEDUCANDO|BENEFICIARIO|SUJE?TO|AGENTE|DENUNCIAD[OA](?:\s+A)?|NOTICIAD[OA]|AUTUAD[OA]|FLAGRANTEAD[OA]|FLAGRANTEADO\s*\(?A\)?|FLAGRANTEADA|SENTENCIAD[OA]|EM\s+APURACAO|POLO\s+PASSIVO|PASSIVO|PACIENTE|PROMOVIDO|DEPRECAD[OA](?:\s+A)?|INFRATOR|CORREU|REQUERIDO|REQUERIDA|APELANTE|APELADA|APELADO|RECORRENTE|RECORRIDA|RECORRIDO\s+A|RECORRIDO|AGRAVANTE|AGRAVADO\s+A|AGRAVADO|EMBARGADO|EMBARGANTE|NOTICIADO|OFENSOR)$/i;
...
function classifyRole(role, area = '') {
    ...
    if (normalizedArea.includes('CRIM') || normalizedArea.includes('PENAL')) {
        ...
        if (/^(AUTOR(?:\s+A)?|AUTORA(?:\s+A)?|REQUERENTE(?:S)?|IMPETRANTE|QUERELANTE|ATIVO|POLO\s+ATIVO(?:\s+PRINCIPAL)?|EXEQUENTE|EXEQTE|REQTE|DEMANDANTE|PROMOVENTE|PARTE\s+AUTORA|RECTE)$/.test(normalizedRole)) {
