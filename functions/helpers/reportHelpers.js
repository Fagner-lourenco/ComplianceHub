...
function selectTopProcessos(caseData, limit = 10) {
    ...
    for (const p of juditRoleSummary) {
        ...
        all.push({
            ...
            polo: p.side || p.personType || null,
            tribunal: p.tribunalAcronym || null,
            vara: p.county || null,
            comarca: p.city || null,
            data: p.distributionDate || null,
            fonte: 'Judit',
    ...
    for (const p of bdcProcessos) {
        ...
        all.push({
            cnj: cnj || 'N/A',
            area: p.courtType || p.cnjBroadSubject || null,
            classe: p.cnjProcedure || p.tipo || null,
            assunto: p.assunto || p.cnjSubject || null,
            status: p.status || null,
            polo: p.polo || p.partyType || null,
            tribunal: p.courtName || null,
            vara: null,
            comarca: p.courtDistrict || null,
            data: p.lastMovementDate || null,
            fonte: 'BigDataCorp',
            isCriminal: !!p.isCriminal || processArea.area === 'CRIMINAL',
    ...
    for (const p of escavadorProcessos) {
        ...
        all.push({
            ...
            status: p.status || null,
            polo: p.polo || p.tipoNormalizado || null,
            tribunal: p.tribunalSigla || null,
            vara: null,
            comarca: p.processCity || null,
            data: p.dataInicio || null,
            fonte: 'Escavador',
