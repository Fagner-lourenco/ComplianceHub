...
const DrawerResumoTab = memo(function DrawerResumoTab({ publicResultLoading, selectedCase, selectedCaseView, has }) {
    const progress = getMacroProgress(selectedCase);
    return (
        <div className="case-detail">
            ...
            {selectedCaseView && (
                <>
                    ...
                    {selectedCase.status !== 'DONE' && <div className="case-detail__section"><h4>Andamento</h4><p style={{ color: progress.color }}>{progress.label}</p></div>}
                </>
            )}
        </div>
    );
...
const DrawerDetalhesTab = memo(function DrawerDetalhesTab({ selectedCase, selectedCaseView, correctionForm, setCorrectionForm, correctionError, correctionSaving, isDemoMode, handleResubmit }) {
    return (
        <div className="case-detail">
...
const DrawerSocialTab = memo(function DrawerSocialTab({ hasSocialProfiles, hasSocialAnalysis, selectedCase, selectedCaseView }) {
    return (
        <div className="case-detail">
            {hasSocialProfiles && (
                <div className="case-detail__section">
...
const DrawerComunicacaoTab = memo(function DrawerComunicacaoTab({ selectedCase }) {
    return (
        <div className="case-detail" style={{ height: '100%', minHeight: 400 }}>
...
const DrawerTimelineTab = memo(function DrawerTimelineTab({ selectedCaseView }) {
    return (
        <div className="case-detail">
            <div className="timeline">
                {(selectedCaseView?.timelineEvents || []).map((event, index) => (
                    <div key={`${event.type}-${index}`} className="timeline__item">
...
const DrawerDossieTab = memo(function DrawerDossieTab({ selectedCaseView, reportAvailability, reportStatus, handleOpenReport }) {
    return (
        <div className="case-detail">
            <div className="case-detail__section">
                <h4>Disponibilidade</h4>
                <p>{reportAvailability.message}</p>
                {selectedCaseView?.reportSlug && <p><strong>Identificador:</strong> {selectedCaseView.reportSlug}</p>}
            </div>
            {reportStatus.message && <p role={reportStatus.state === 'error' ? 'alert' : 'status'} style={{ color: reportStatus.state === 'error' ? 'var(--red-600)' : reportStatus.state === 'success' ? 'var(--green-700)' : 'var(--text-secondary)' }}>{reportStatus.message}</p>}
            <div className="case-detail__section">
                <p>Esta prévia é resumida. Para leitura, impressão ou compartilhamento externo, abra o dossiê autenticado.</p>
                <button className="btn-primary" style={{ fontSize: '.8125rem' }} onClick={handleOpenReport} disabled={reportStatus.state === 'loading' || !reportAvailability.available}>{reportStatus.state === 'loading' ? 'Abrindo...' : 'Abrir dossiê completo'}</button>
            </div>
        </div>
    );
});
...
export default function SolicitacoesPage() {
    ...
    }, [location.pathname, navigate, reportAvailability.available, reportAvailability.message, selectedCaseView]);
    ...
    }, [correctionForm, selectedCase, user]);
    ...
    const drawerTabs = useMemo(() => {
        ...
        const tabs = [
            {
                label: 'Resumo',
                content: <DrawerResumoTab publicResultLoading={publicResultLoading} selectedCase={selectedCase} selectedCaseView={selectedCaseView} has={has} />,
            },
            {
                label: 'Detalhes',
                content: <DrawerDetalhesTab selectedCase={selectedCase} selectedCaseView={selectedCaseView} correctionForm={correctionForm} setCorrectionForm={setCorrectionForm} correctionError={correctionError} correctionSaving={correctionSaving} isDemoMode={isDemoMode} handleResubmit={handleResubmit} />,
            },
        ];
        if (shouldShowSocialTab) {
            tabs.push({
                label: hasSocialAnalysis ? 'Informações digitais' : 'Perfis informados',
                content: <DrawerSocialTab hasSocialProfiles={hasSocialProfiles} hasSocialAnalysis={hasSocialAnalysis} selectedCase={selectedCase} selectedCaseView={selectedCaseView} />,
            });
        }
        tabs.push(
            {
                label: 'Comunicacao',
                badge: selectedCase?.clientUnreadMessages > 0 ? selectedCase.clientUnreadMessages : null,
                content: <DrawerComunicacaoTab selectedCase={selectedCase} />,
            },
            {
                label: 'Timeline',
                content: <DrawerTimelineTab selectedCaseView={selectedCaseView} />,
            },
            {
                label: 'Dossiê',
                content: <DrawerDossieTab selectedCaseView={selectedCaseView} reportAvailability={reportAvailability} reportStatus={reportStatus} handleOpenReport={handleOpenReport} />,
            }
        );
        return tabs;
    }, [correctionError, correctionForm, correctionSaving, has, handleOpenReport, handleResubmit, hasSocialAnalysis, hasSocialProfiles, isDemoMode, publicResultLoading, reportAvailability, reportStatus, selectedCase, selectedCaseView, shouldShowSocialTab]);

    const handleClickAll = useCallback(() => { setStatusFilter('ALL'); setVerdictFilter('ALL'); }, []);
    const handleClickDone = useCallback(() => setStatusFilter('DONE'), []);
    const handleClickPending = useCallback(() => setStatusFilter('PENDING'), []);
    const handleClickCorrections = useCallback(() => setStatusFilter('CORRECTION_NEEDED'), []);
    const handleClickNotRecommended = useCallback(() => setVerdictFilter('NOT_RECOMMENDED'), []);

    const renderCard = useCallback((caseData) => (
    ...
    return (
        <PageShell size="default" className="solicitacoes-page">
            ...
            />
            <div className="solicitacoes-page__kpis"><KpiCard label="Solicitações encontradas" value={stats.total} color="neutral" onClick={handleClickAll} /><KpiCard label="Concluídas" value={stats.done} color="green" onClick={handleClickDone} /><KpiCard label="Aguardando início" value={stats.pending} color="yellow" onClick={handleClickPending} />{stats.corrections > 0 && <KpiCard label="Aguardando correção" value={stats.corrections} color="red" onClick={handleClickCorrections} />}<KpiCard label="Não recomendados" value={stats.notRecommended} color="red" onClick={handleClickNotRecommended} /></div>
            ...
            >
                <div className="solicitacoes-page__table-wrapper"><table className="data-table" aria-label="Solicitações de análise cadastral"><thead><tr><th className="data-table__th data-table__th--sortable" scope="col" onClick={() => handleSort('candidateName')} aria-sort={sortField === 'candidateName' ? (sortDir === 'asc' ? 'ascending' : 'descending') : 'none'}>Nome {sortField === 'candidateName' && (sortDir === 'asc' ? '↑' : '↓')}</th><th className="data-table__th" scope="col">CPF</th><th className="data-table__th data-table__th--sortable" scope="col" onClick={() => handleSort('createdAt')} aria-sort={sortField === 'createdAt' ? (sortDir === 'asc' ? 'ascending' : 'descending') : 'none'}>Data {sortField === 'createdAt' && (sortDir === 'asc' ? '↑' : '↓')}</th><th className="data-table__th" scope="col">Status</th>{has('criminal') && <th className="data-table__th" scope="col">Criminal</th>}{has('labor') && <th className="data-table__th" scope="col">Trabalhista</th>}{has('warrant') && <th className="data-table__th" scope="col">Mandado</th>}{has('osint') && <th className="data-table__th" scope="col">Perfis públicos</th>}{has('social') && <th className="data-table__th" scope="col">Social</th>}{has('digital') && <th className="data-table__th" scope="col">Digital</th>}<th className="data-table__th" scope="col">Resultado</th><th className="data-table__th" scope="col">Indicadores</th></tr></thead><tbody>{loading && Array.from({ length: 5 }, (_, i) => (<tr key={`sk-${i}`} aria-hidden="true"><td className="data-table__td"><div className="skeleton skeleton--text" style={{ width: `${55 + (i % 4) * 10}%` }} /></td><td className="data-table__td"><div className="skeleton skeleton--text" style={{ width: 90 }} /></td><td className="data-table__td"><div className="skeleton skeleton--text" style={{ width: 72 }} /></td><td className="data-table__td"><div className="skeleton" style={{ width: 72, height: 22, borderRadius: 20 }} /></td><td className="data-table__td" colSpan={7}><div className="skeleton skeleton--text" style={{ width: '40%' }} /></td></tr>))}{!loading && error && <tr><td colSpan={12} className="data-table__empty" style={{ color: 'var(--red-700)' }}>{extractErrorMessage(error, 'Nao foi possivel carregar suas solicitacoes agora.')}</td></tr>}{!loading && !error && paginatedCases.map((caseData) => <tr key={caseData.id} className={`data-table__row ${heatmapMode ? `data-table__row--heat-${caseData.riskLevel || 'none'}` : ''} ${selectedCase?.id === caseData.id ? 'data-table__row--selected' : ''}`} onClick={() => setSelectedCase(caseData)}><td className="data-table__td data-table__td--name">{caseData.candidateName}</td><td className="data-table__td data-table__td--mono">{caseData.cpfMasked}</td><td className="data-table__td">{formatDate(caseData.createdAt)}</td><td className="data-table__td"><StatusBadge status={caseData.status} />{caseData.status !== 'DONE' && <span style={{ display: 'block', fontSize: '.6875rem', color: getMacroProgress(caseData).color, fontStyle: 'italic', marginTop: 2 }}>{getMacroProgress(caseData).label}</span>}</td>{has('criminal') && <td className="data-table__td">{caseData.status === 'DONE' ? <RiskChip value={caseData.criminalFlag} /> : <span className="data-table__hidden">—</span>}</td>}{has('labor') && <td className="data-table__td">{caseData.status === 'DONE' ? <RiskChip value={caseData.laborFlag} /> : <span className="data-table__hidden">—</span>}</td>}{has('warrant') && <td className="data-table__td">{caseData.status === 'DONE' ? <RiskChip value={caseData.warrantFlag} /> : <span className="data-table__hidden">—</span>}</td>}{has('osint') && <td className="data-table__td">{caseData.status === 'DONE' ? <RiskChip value={caseData.osintLevel} /> : <span className="data-table__hidden">—</span>}</td>}{has('social') && <td className="data-table__td">{caseData.status === 'DONE' ? <RiskChip value={caseData.socialStatus} /> : <span className="data-table__hidden">—</span>}</td>}{has('digital') && <td className="data-table__td">{caseData.status === 'DONE' ? <RiskChip value={caseData.digitalFlag} /> : <span className="data-table__hidden">—</span>}</td>}<td className="data-table__td">{caseData.status === 'DONE' ? <RiskChip value={caseData.finalVerdict} bold size="md" /> : <span className="data-table__hidden">—</span>}</td><td className="data-table__td">{caseData.hasNotes && '📝 '}{caseData.hasEvidence && '📎'}</td></tr>)}{!loading && !error && filteredCases.length === 0 && <tr><td colSpan={12} className="data-table__empty"><div className="empty-state"><span className="empty-state__icon" aria-hidden="true"><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M8 4v4M16 4v4M2 8h20M8 12h8M8 16h4"/></svg></span><p>Nenhuma solicitacao encontrada.</p><button className="empty-state__btn" onClick={() => { setStatusFilter('ALL'); setVerdictFilter('ALL'); setSearchTerm(''); }}>Limpar filtros</button></div></td></tr>}</tbody></table></div>
            </MobileDataCardList>
            <PaginationControls
