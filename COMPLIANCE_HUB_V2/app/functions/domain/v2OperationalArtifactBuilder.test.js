...
} from './v2OperationalArtifactBuilder.js';

describe('v2OperationalArtifactBuilder', () => {
    ...
    it('retorna arrays vazios quando caseData nao tem fontes', () => {
        const artifacts = buildOperationalArtifactsForCase({
            caseId: 'case-empty',
            caseData: { tenantId: 'tenant-1' },
        });
        expect(artifacts.providerRequests).toEqual([]);
        expect(artifacts.rawSnapshots).toEqual([]);
        expect(artifacts.evidenceItems).toEqual([]);
        expect(artifacts.riskSignals).toEqual([]);
    });

    it('buildProviderRequestsForCase retorna vazio quando nao ha sources', () => {
        const requests = buildProviderRequestsForCase({ caseId: 'case-empty', caseData: { tenantId: 't1' } });
        expect(requests).toEqual([]);
    });

    it('buildRawSnapshotsForCase retorna vazio quando nao ha providerRequests', () => {
        const snapshots = buildRawSnapshotsForCase({ caseId: 'case-empty', caseData: { tenantId: 't1' }, providerRequests: [] });
