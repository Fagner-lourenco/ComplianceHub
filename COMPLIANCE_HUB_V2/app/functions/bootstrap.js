...
} = require('./domain/dossierSchema.js');
const {
    resolveSubject,
...
} = require('./domain/v2ProviderLedger.js');
const {
    getFreshnessPolicy,
} = require('./domain/v2FreshnessPolicyResolver.js');
const {
    createRawSnapshot,
} = require('./domain/v2RawSnapshot.js');
const {
    processWatchlists,
...
const DEFAULT_BIGDATACORP_CONFIG = {
    ...
    presetKey: null,          // 'compliance', 'dossier_pf_full', 'dossier_pj_full', etc.
    phases: {
        ...
        occupation: true,     // R$0.05 — employment/profession history
        // Extended datasets (V2 orchestrator)
        phones_extended: false,
        addresses_extended: false,
        emails_extended: false,
        online_presence: false,
        financial_data: false,
        class_organization: false,
        relationships: false,
        activity_indicators: false,
        company_evolution: false,
        owners_kyc: false,
        government_debtors: false,
        collections: false,
        historical_basic_data: false,
        financial_risk: false,
        indebtedness_question: false,
        media_profile_and_exposure: false,
        lawsuits_distribution_data: false,
        profession_data: false,
        // PJ datasets
        owners_lawsuits: false,
        employees_kyc: false,
        economic_group_kyc: false,
        dynamic_qsa_data: false,
        history_basic_data: false,
        merchant_category_data: false,
        syndicate_agreements: false,
        social_conscience: false,
        online_ads: false,
    },
    gate: { minNameSimilarity: 0.7 },
    processLimit: 100,        // Max processes to return per query
    ownersKycLimit: 50,       // Max owners for owners_kyc dataset
};
...
/* =========================================================
   BIGDATACORP — Normalization & Update (extracted for reuse)
   ========================================================= */

async function normalizeAndUpdateBdcResult(caseRef, caseId, caseData, result, bdcConfig, options = {}, providerRequestId = null, rawSnapshotIds = []) {
    const cpf = (caseData.cpf || '').replace(/\D/g, '');
    ...
    if (!options.skipGate && updatePayload.bigdatacorpCpfStatus) {
        ...
        if (!gatePassed) {
            ...
            if (providerRequestId) {
                await resolveProviderRequest(providerRequestId, {
                    status: 'blocked',
                    actualCostBRL: costBRL,
                    rawSnapshotIds,
                    errorMessage: `Gate bloqueado: ${gateReason}`,
                }).catch((err) => console.warn(`Case ${caseId} [BigDataCorp]: resolveProviderRequest error:`, err.message));
            }
            await materializeModuleRunsFromCaseRef(caseRef, caseId, caseData);
    ...
    // ─── V2 Extended Datasets (from orchestrator) ───
    if (phases.phones_extended !== false && result.resultEntry?.ExtendedPhones) {
        const { normalizeContacts } = require('./normalizers/bigdatacorp/basicData');
        const contacts = normalizeContacts({ ExtendedPhones: result.resultEntry.ExtendedPhones });
        updatePayload.bigdatacorpPhones = contacts.phones;
        updatePayload.bigdatacorpPhoneStats = contacts.stats?.phones;
        sources.phones_extended = { provider: 'bigdatacorp', dataset: 'phones_extended', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.addresses_extended !== false && result.resultEntry?.ExtendedAddresses) {
        const { normalizeContacts } = require('./normalizers/bigdatacorp/basicData');
        const contacts = normalizeContacts({ ExtendedAddresses: result.resultEntry.ExtendedAddresses });
        updatePayload.bigdatacorpAddresses = contacts.addresses;
        updatePayload.bigdatacorpAddressStats = contacts.stats?.addresses;
        sources.addresses_extended = { provider: 'bigdatacorp', dataset: 'addresses_extended', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.emails_extended !== false && result.resultEntry?.ExtendedEmails) {
        const { normalizeContacts } = require('./normalizers/bigdatacorp/basicData');
        const contacts = normalizeContacts({ ExtendedEmails: result.resultEntry.ExtendedEmails });
        updatePayload.bigdatacorpEmails = contacts.emails;
        updatePayload.bigdatacorpEmailStats = contacts.stats?.emails;
        sources.emails_extended = { provider: 'bigdatacorp', dataset: 'emails_extended', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.online_presence !== false && result.resultEntry?.OnlinePresence) {
        const { normalizeOnlinePresence, buildOnlinePresenceEvidenceContent } = require('./normalizers/bigdatacorp/onlinePresence');
        const presence = normalizeOnlinePresence(result.resultEntry.OnlinePresence);
        updatePayload.bigdatacorpOnlinePresence = presence;
        updatePayload.bigdatacorpOnlinePresenceEvidence = buildOnlinePresenceEvidenceContent(presence);
        sources.online_presence = { provider: 'bigdatacorp', dataset: 'online_presence', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.financial_data !== false && result.resultEntry?.FinantialData) {
        updatePayload.bigdatacorpFinancialData = result.resultEntry.FinantialData;
        sources.financial_data = { provider: 'bigdatacorp', dataset: 'financial_data', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.government_debtors !== false && result.resultEntry?.GovernmentDebtors) {
        const { normalizeGovernmentDebtors, buildGovernmentDebtorsEvidenceContent } = require('./normalizers/bigdatacorp/governmentDebtors');
        const debtors = normalizeGovernmentDebtors(result.resultEntry.GovernmentDebtors);
        updatePayload.bigdatacorpGovernmentDebtors = debtors;
        updatePayload.bigdatacorpGovernmentDebtorsEvidence = buildGovernmentDebtorsEvidenceContent(debtors);
        sources.government_debtors = { provider: 'bigdatacorp', dataset: 'government_debtors', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.collections !== false && result.resultEntry?.Collections) {
        const { normalizeCollections, buildCollectionsEvidenceContent } = require('./normalizers/bigdatacorp/collections');
        const collections = normalizeCollections(result.resultEntry.Collections);
        updatePayload.bigdatacorpCollections = collections;
        updatePayload.bigdatacorpCollectionsEvidence = buildCollectionsEvidenceContent(collections);
        sources.collections = { provider: 'bigdatacorp', dataset: 'collections', consultedAt: new Date().toISOString() };
        costBRL += 0.07;
    }

    if (phases.relationships !== false && result.resultEntry?.Relationships) {
        const { normalizeRelationships, buildRelationshipEvidenceContent } = require('./normalizers/bigdatacorp/relationships');
        const rels = normalizeRelationships(result.resultEntry.Relationships);
        updatePayload.bigdatacorpRelationships = rels;
        updatePayload.bigdatacorpRelationshipsEvidence = buildRelationshipEvidenceContent(rels);
        sources.relationships = { provider: 'bigdatacorp', dataset: 'relationships', consultedAt: new Date().toISOString() };
        costBRL += 0.03;
    }

    if (phases.owners_kyc !== false && result.resultEntry?.OwnersKycData) {
        const { normalizeOwnersKyc, buildOwnersKycEvidenceContent } = require('./normalizers/bigdatacorp/ownersKyc');
        const ownersKyc = normalizeOwnersKyc(result.resultEntry.OwnersKycData);
        updatePayload.bigdatacorpOwnersKyc = ownersKyc;
        updatePayload.bigdatacorpOwnersKycEvidence = buildOwnersKycEvidenceContent(ownersKyc);
        sources.owners_kyc = { provider: 'bigdatacorp', dataset: 'owners_kyc', consultedAt: new Date().toISOString() };
        costBRL += 0.09;
    }

    if (phases.activity_indicators !== false && result.resultEntry?.ActivityIndicators) {
        ...
        const indicators = normalizeActivityIndicators(result.resultEntry.ActivityIndicators);
        updatePayload.bigdatacorpActivityIndicators = indicators;
        updatePayload.bigdatacorpActivityIndicatorsEvidence = buildActivityIndicatorsEvidenceContent(indicators);
        sources.activity_indicators = { provider: 'bigdatacorp', dataset: 'activity_indicators', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.company_evolution !== false && result.resultEntry?.CompanyEvolutionData) {
        const { normalizeCompanyEvolution, buildCompanyEvolutionEvidenceContent } = require('./normalizers/bigdatacorp/companyEvolution');
        const evolution = normalizeCompanyEvolution(result.resultEntry.CompanyEvolutionData);
        updatePayload.bigdatacorpCompanyEvolution = evolution;
        updatePayload.bigdatacorpCompanyEvolutionEvidence = buildCompanyEvolutionEvidenceContent(evolution);
        sources.company_evolution = { provider: 'bigdatacorp', dataset: 'company_evolution', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.class_organization !== false && result.resultEntry?.Memberships) {
        updatePayload.bigdatacorpMemberships = result.resultEntry.Memberships;
        sources.class_organization = { provider: 'bigdatacorp', dataset: 'class_organization', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.historical_basic_data !== false && result.resultEntry?.HistoricalBasicData) {
        const { normalizeHistoricalBasicData, buildHistoricalBasicDataEvidence } = require('./normalizers/bigdatacorp/historicalBasicData');
        const hist = normalizeHistoricalBasicData(result.resultEntry.HistoricalBasicData);
        updatePayload.bigdatacorpHistoricalBasicData = hist;
        updatePayload.bigdatacorpHistoricalBasicDataEvidence = buildHistoricalBasicDataEvidence(hist);
        sources.historical_basic_data = { provider: 'bigdatacorp', dataset: 'historical_basic_data', consultedAt: new Date().toISOString() };
        costBRL += 0.03;
    }

    if (phases.financial_risk !== false && result.resultEntry?.FinancialRisk) {
        const { normalizeFinancialRisk, buildFinancialRiskEvidence } = require('./normalizers/bigdatacorp/risk');
        const risk = normalizeFinancialRisk(result.resultEntry.FinancialRisk);
        updatePayload.bigdatacorpFinancialRisk = risk;
        updatePayload.bigdatacorpFinancialRiskEvidence = buildFinancialRiskEvidence(risk);
        sources.financial_risk = { provider: 'bigdatacorp', dataset: 'financial_risk', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.indebtedness_question !== false && result.resultEntry?.IndebtednessQuestion) {
        const { normalizeIndebtedness, buildIndebtednessEvidence } = require('./normalizers/bigdatacorp/risk');
        const debt = normalizeIndebtedness(result.resultEntry.IndebtednessQuestion);
        updatePayload.bigdatacorpIndebtedness = debt;
        updatePayload.bigdatacorpIndebtednessEvidence = buildIndebtednessEvidence(debt);
        sources.indebtedness_question = { provider: 'bigdatacorp', dataset: 'indebtedness_question', consultedAt: new Date().toISOString() };
        costBRL += 0.09;
    }

    if (phases.media_profile_and_exposure !== false && result.resultEntry?.MediaProfileAndExposure) {
        const { normalizeMediaProfile, buildMediaProfileEvidence } = require('./normalizers/bigdatacorp/mediaProfile');
        const profile = normalizeMediaProfile(result.resultEntry.MediaProfileAndExposure);
        updatePayload.bigdatacorpMediaProfile = profile;
        updatePayload.bigdatacorpMediaProfileEvidence = buildMediaProfileEvidence(profile);
        sources.media_profile_and_exposure = { provider: 'bigdatacorp', dataset: 'media_profile_and_exposure', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.lawsuits_distribution_data !== false && result.resultEntry?.LawsuitsDistributionData) {
        ...
        const dist = normalizeLawsuitsDistribution(result.resultEntry.LawsuitsDistributionData);
        updatePayload.bigdatacorpLawsuitsDistribution = dist;
        updatePayload.bigdatacorpLawsuitsDistributionEvidence = buildLawsuitsDistributionEvidence(dist);
        sources.lawsuits_distribution_data = { provider: 'bigdatacorp', dataset: 'lawsuits_distribution_data', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    // PJ-only datasets
    if (phases.owners_lawsuits !== false && result.resultEntry?.OwnersLawsuits) {
        const { normalizeOwnersLawsuits, buildOwnersLawsuitsEvidence } = require('./normalizers/bigdatacorp/ownersLawsuits');
        const data = normalizeOwnersLawsuits(result.resultEntry.OwnersLawsuits);
        updatePayload.bigdatacorpOwnersLawsuits = data;
        updatePayload.bigdatacorpOwnersLawsuitsEvidence = buildOwnersLawsuitsEvidence(data);
        sources.owners_lawsuits = { provider: 'bigdatacorp', dataset: 'owners_lawsuits', consultedAt: new Date().toISOString() };
        costBRL += 0.13;
    }

    if (phases.employees_kyc !== false && result.resultEntry?.EmployeesKycData) {
        const { normalizeEmployeesKyc, buildEmployeesKycEvidence } = require('./normalizers/bigdatacorp/employeesKyc');
        const data = normalizeEmployeesKyc(result.resultEntry.EmployeesKycData);
        updatePayload.bigdatacorpEmployeesKyc = data;
        updatePayload.bigdatacorpEmployeesKycEvidence = buildEmployeesKycEvidence(data);
        sources.employees_kyc = { provider: 'bigdatacorp', dataset: 'employees_kyc', consultedAt: new Date().toISOString() };
        costBRL += 0.41;
    }

    if (phases.economic_group_kyc !== false && result.resultEntry?.EconomicGroupKycData) {
        const { normalizeEconomicGroupKyc, buildEconomicGroupKycEvidence } = require('./normalizers/bigdatacorp/economicGroupKyc');
        const data = normalizeEconomicGroupKyc(result.resultEntry.EconomicGroupKycData);
        updatePayload.bigdatacorpEconomicGroupKyc = data;
        updatePayload.bigdatacorpEconomicGroupKycEvidence = buildEconomicGroupKycEvidence(data);
        sources.economic_group_kyc = { provider: 'bigdatacorp', dataset: 'economic_group_kyc', consultedAt: new Date().toISOString() };
        costBRL += 0.41;
    }

    if (phases.dynamic_qsa_data !== false && result.resultEntry?.DynamicQSAData) {
        const { normalizeDynamicQsa, buildDynamicQsaEvidence } = require('./normalizers/bigdatacorp/dynamicQsa');
        const data = normalizeDynamicQsa(result.resultEntry.DynamicQSAData);
        updatePayload.bigdatacorpDynamicQsa = data;
        updatePayload.bigdatacorpDynamicQsaEvidence = buildDynamicQsaEvidence(data);
        sources.dynamic_qsa_data = { provider: 'bigdatacorp', dataset: 'dynamic_qsa_data', consultedAt: new Date().toISOString() };
        costBRL += 0.09;
    }

    if (phases.history_basic_data !== false && result.resultEntry?.HistoryBasicData) {
        const { normalizeHistoryBasicData, buildHistoryBasicDataEvidence } = require('./normalizers/bigdatacorp/historyBasicData');
        const data = normalizeHistoryBasicData(result.resultEntry.HistoryBasicData);
        updatePayload.bigdatacorpHistoryBasicData = data;
        updatePayload.bigdatacorpHistoryBasicDataEvidence = buildHistoryBasicDataEvidence(data);
        sources.history_basic_data = { provider: 'bigdatacorp', dataset: 'history_basic_data', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.merchant_category_data !== false && result.resultEntry?.MerchantCategoryData) {
        const { normalizeMerchantCategory, buildMerchantCategoryEvidence } = require('./normalizers/bigdatacorp/merchantCategory');
        const data = normalizeMerchantCategory(result.resultEntry.MerchantCategoryData);
        updatePayload.bigdatacorpMerchantCategory = data;
        updatePayload.bigdatacorpMerchantCategoryEvidence = buildMerchantCategoryEvidence(data);
        sources.merchant_category_data = { provider: 'bigdatacorp', dataset: 'merchant_category_data', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.syndicate_agreements !== false && result.resultEntry?.SyndicateAgreements) {
        const { normalizeSyndicateAgreements, buildSyndicateAgreementsEvidence } = require('./normalizers/bigdatacorp/syndicateAgreements');
        const data = normalizeSyndicateAgreements(result.resultEntry.SyndicateAgreements);
        updatePayload.bigdatacorpSyndicateAgreements = data;
        updatePayload.bigdatacorpSyndicateAgreementsEvidence = buildSyndicateAgreementsEvidence(data);
        sources.syndicate_agreements = { provider: 'bigdatacorp', dataset: 'syndicate_agreements', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.social_conscience !== false && result.resultEntry?.SocialConsciousness) {
        const { normalizeSocialConscience, buildSocialConscienceEvidence } = require('./normalizers/bigdatacorp/socialConscience');
        const data = normalizeSocialConscience(result.resultEntry.SocialConsciousness);
        updatePayload.bigdatacorpSocialConscience = data;
        updatePayload.bigdatacorpSocialConscienceEvidence = buildSocialConscienceEvidence(data);
        sources.social_conscience = { provider: 'bigdatacorp', dataset: 'social_conscience', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }

    if (phases.online_ads !== false && result.resultEntry?.OnlineAds) {
        const { normalizeOnlineAds, buildOnlineAdsEvidence } = require('./normalizers/bigdatacorp/onlineAds');
        const data = normalizeOnlineAds(result.resultEntry.OnlineAds);
        updatePayload.bigdatacorpOnlineAds = data;
        updatePayload.bigdatacorpOnlineAdsEvidence = buildOnlineAdsEvidence(data);
        sources.online_ads = { provider: 'bigdatacorp', dataset: 'online_ads', consultedAt: new Date().toISOString() };
        costBRL += 0.05;
    }
    ...
    // Resolve ProviderRequest as completed
    if (providerRequestId) {
        await resolveProviderRequest(providerRequestId, {
            status: 'completed',
            actualCostBRL: costBRL,
...
async function runBigDataCorpEnrichmentPhase(caseRef, caseId, caseData, bdcConfig, options = {}) {
    ...
    const subjectType = caseData.subjectType || 'pf';
    const cpf = (caseData.cpf || '').replace(/\D/g, '');
    const cnpj = (caseData.cnpj || '').replace(/\D/g, '');
    const doc = subjectType === 'pj' ? cnpj : cpf;
    const docLabel = subjectType === 'pj' ? 'CNPJ' : 'CPF';
    const docLen = subjectType === 'pj' ? 14 : 11;
    if (doc.length !== docLen) {
        const error = `${docLabel} invalido.`;
        await caseRef.update({
    ...
    try {
        ...
        // Determine datasets for reuse hash
        const reuseDatasets = bdcConfig.presetKey
            ? getDatasetsForPreset(bdcConfig.presetKey, subjectType)
            : ['basic_data', 'processes', 'kyc', 'occupation_data'];

        // Check freshness/reuse before querying
        const requestHash = require('./domain/v2ProviderLedger.js').hashRequest({ [subjectType === 'pj' ? 'cnpj' : 'cpf']: doc, datasets: reuseDatasets });
        const reusable = await findReusableProviderRequest({
            tenantId,
            provider: 'bigdatacorp',
            dataset: 'combined',
            requestHash,
            maxAgeMs: bdcConfig.freshnessMs || 86400000,
        });

        if (reusable) {
            console.log(`Case ${caseId} [BigDataCorp]: reusing snapshot from ${reusable.id}`);
            providerRequestId = reusable.id;
            await markProviderRequestReused(providerRequestId, { reusedSnapshotId: reusable.rawSnapshotIds?.[0] || null, reason: 'freshness_valid' });
            // Use previous result without new API call
            const previousSnap = await db.collection('rawSnapshots')
                .where('providerRequestId', '==', providerRequestId)
                .orderBy('createdAt', 'desc')
                .limit(1)
                .get();
            const previousPayload = previousSnap.empty ? null : previousSnap.docs[0].data()?.payload;
            if (previousPayload) {
                const entry = previousPayload.Result?.[0] || {};
                const result = {
                    raw: previousPayload,
                    resultEntry: entry,
                    basicData: entry.BasicData || null,
                    processes: entry.Processes || null,
                    kycData: entry.KycData || null,
                    professionData: entry.ProfessionData || null,
                    elapsedMs: 0,
                };
                // Skip to normalization
                const reusedSnapshotIds = reusable.rawSnapshotIds || [];
                return await normalizeAndUpdateBdcResult(caseRef, caseId, caseData, result, bdcConfig, options, providerRequestId, reusedSnapshotIds);
            }
        }

        // Create ProviderRequest before querying
        const queryDatasets = bdcConfig.presetKey
            ? getDatasetsForPreset(bdcConfig.presetKey, subjectType)
            : ['basic_data', 'processes', 'kyc', 'occupation_data'];
        const estimatedCost = bdcConfig.presetKey
            ? require('./adapters/bigdatacorpCatalog').estimatePresetCost(bdcConfig.presetKey, subjectType)
            : 0.20;
        const providerReq = await createProviderRequest({
            tenantId,
            caseId,
            subjectId: caseData.candidateId || null,
            provider: 'bigdatacorp',
            dataset: 'combined',
            query: { [subjectType === 'pj' ? 'cnpj' : 'cpf']: doc, datasets: queryDatasets },
            estimatedCostBRL: estimatedCost,
            requestedBy: 'system:enrichBigDataCorpOnCase',
        });
        providerRequestId = providerReq.id;

        console.log(`Case ${caseId} [BigDataCorp]: querying ${docLabel}=${doc}, processLimit=${processLimit}, datasets=${queryDatasets.join(',')}`);

        let result;
        let usedOrchestrator = false;

        // ─── V2 Orchestrator path (when presetKey is configured) ───
        if (bdcConfig.presetKey) {
            usedOrchestrator = true;
            const subjectType = caseData.subjectType || 'pf';
            const doc = subjectType === 'pj' ? (caseData.cnpj || '').replace(/\D/g, '') : cpf;

            const orchResult = await enrichFromPreset(doc, credentials, {
                presetKey: bdcConfig.presetKey,
                subjectType,
                processLimit,
                ownersKycLimit: bdcConfig.ownersKycLimit || 50,
            });

            if (orchResult.status === 'failed') {
                throw new Error(orchResult.errors.join('; '));
            }

            // Convert orchestrator result to legacy format expected by normalizeAndUpdateBdcResult
            result = {
                raw: orchResult.raw,
                resultEntry: orchResult.resultEntry,
                basicData: orchResult.resultEntry?.BasicData || null,
                processes: orchResult.resultEntry?.Processes || null,
                kycData: orchResult.resultEntry?.KycData || null,
                professionData: orchResult.resultEntry?.ProfessionData || null,
                elapsedMs: orchResult.elapsedMs,
            };

            // Create RawSnapshot with the raw payload
            rawSnapshotIds = [];
            try {
                const snapResult = await createRawSnapshot({
                    tenantId,
                    caseId,
                    subjectId: caseData.candidateId || null,
                    providerRequestId,
                    provider: 'bigdatacorp',
                    endpoint: subjectType === 'pj' ? '/empresas' : '/pessoas',
                    datasets: orchResult.raw?.Result?.[0] ? Object.keys(orchResult.raw.Result[0]) : ['combined'],
                    payload: orchResult.raw || {},
                    queriedAt: new Date().toISOString(),
                });
                rawSnapshotIds = [snapResult.id];
            } catch (snapErr) {
                console.warn(`Case ${caseId} [BigDataCorp]: failed to create rawSnapshot:`, snapErr.message);
            }
        } else {
            // ─── Legacy path (4 datasets) ───
            result = await queryBigDataCorpCombined(cpf, credentials, { processLimit });
            ...
            try {
                const snapResult = await createRawSnapshot({
                    ...
                    provider: 'bigdatacorp',
                    endpoint: '/pessoas',
                    datasets: ['basic_data', 'processes', 'kyc', 'occupation_data'],
                    payload: result.raw || {},
                    queriedAt: new Date().toISOString(),
                });
                rawSnapshotIds = [snapResult.id];
        ...
        if (usedOrchestrator) {
            console.log(`Case ${caseId} [BigDataCorp]: orchestrator preset=${bdcConfig.presetKey}, datasets=${Object.keys(result.resultEntry || {}).join(',')}`);
        }

        return await normalizeAndUpdateBdcResult(caseRef, caseId, caseData, result, bdcConfig, options, providerRequestId, rawSnapshotIds);
    } catch (err) {
        ...
        if (providerRequestId) {
            await resolveProviderRequest(providerRequestId, {
                ...
                errorMessage: errMsg,
                rawSnapshotIds,
            }).catch((err2) => console.warn(`Case ${caseId} [BigDataCorp]: resolveProviderRequest error:`, err2.message));
        }
        await materializeModuleRunsFromCaseRef(caseRef, caseId, caseData);
        return { status: 'FAILED', error: errMsg };
    }
...
exports.listTenantUsers = onCall(
    ...
    async (request) => {
        ...
        const tenantId = String(request.data?.tenantId || '').trim() || callerProfile.tenantId || null;
        if (callerProfile.role !== 'admin' && callerProfile.role !== 'supervisor' && callerProfile.tenantId !== tenantId) {
            throw new HttpsError('permission-denied', 'Sem permissao para listar usuarios deste tenant.');
        }
...
exports.createTenantUser = onCall(
    ...
    async (request) => {
        ...
        if (callerProfile.role !== 'admin' && callerProfile.role !== 'supervisor') {
            throw new HttpsError('permission-denied', 'Apenas administradores podem criar usuarios.');
        }
        ...
        const tenantId = String(request.data?.tenantId || '').trim() || callerProfile.tenantId;
        const tenantName = callerProfile.tenantName || tenantId;
...
exports.updateTenantUser = onCall(
    ...
    async (request) => {
        ...
        const callerProfile = await getOpsUserProfile(uid);
        if (callerProfile.role !== 'admin' && callerProfile.role !== 'supervisor') {
            throw new HttpsError('permission-denied', 'Apenas administradores podem modificar usuarios.');
        }
        ...
        if (callerProfile.role !== 'admin' && targetProfile.tenantId !== callerProfile.tenantId) {
...
exports.v2PreviewBigDataCorp = onCall(
    { region: 'southamerica-east1', timeoutSeconds: 60, secrets: [bigdatacorpAccessToken, bigdatacorpTokenId] },
    async (request) => {
        const uid = request.auth?.uid;
        if (!uid) throw new HttpsError('unauthenticated', 'Autenticacao necessaria.');
        ...
        const allowedRoles = ['analyst', 'senior_analyst', 'opsManager', 'admin'];
        if (!allowedRoles.includes(role)) {
            throw new HttpsError('permission-denied', 'Acesso restrito a operadores autorizados.');
        }

        const { cpf, cnpj, datasets = ['basic_data', 'processes', 'kyc', 'occupation_data'], tenantId } = request.data || {};
        const cpfDigits = String(cpf || '').replace(/\D/g, '');
        const cnpjDigits = String(cnpj || '').replace(/\D/g, '');
        const doc = cnpjDigits.length === 14 ? cnpjDigits : (cpfDigits.length === 11 ? cpfDigits : null);
        const subjectType = cnpjDigits.length === 14 ? 'pj' : 'pf';

        if (!doc) {
            throw new HttpsError('invalid-argument', 'CPF ou CNPJ invalido.');
        }
        ...
        const tokenId = bigdatacorpTokenId.value();
        if (!accessToken || !tokenId) {
            throw new HttpsError('failed-precondition', 'Credenciais BigDataCorp nao configuradas.');
        }

        const credentials = { accessToken, tokenId };
        let result;
        try {
            result = await queryBigDataCorpCombined(doc, credentials, {
                datasets,
                subjectType,
                processLimit: datasets.includes('processes') ? 100 : 0,
            });
        } catch (err) {
            throw new HttpsError('internal', `Erro na consulta BigDataCorp: ${err.message}`);
        }

        const normalized = {};
        const entry = result.resultEntry || {};
        if (datasets.includes('basic_data')) {
            Object.assign(normalized, normalizeBigDataCorpBasicData(result.basicData));
        }
        if (datasets.includes('processes')) {
            Object.assign(normalized, normalizeBigDataCorpProcesses(result.processes, doc));
        }
        if (datasets.includes('kyc')) {
            Object.assign(normalized, normalizeBigDataCorpKyc(result.kycData));
        }
        if (datasets.includes('occupation_data')) {
            Object.assign(normalized, normalizeBigDataCorpProfession(result.professionData));
        }

        // Extended datasets (V2 orchestrator)
        if (datasets.includes('phones_extended') && entry.ExtendedPhones) {
            const { normalizeContacts } = require('./normalizers/bigdatacorp/basicData');
            Object.assign(normalized, normalizeContacts({ ExtendedPhones: entry.ExtendedPhones }));
        }
        if (datasets.includes('addresses_extended') && entry.ExtendedAddresses) {
            const { normalizeContacts } = require('./normalizers/bigdatacorp/basicData');
            Object.assign(normalized, normalizeContacts({ ExtendedAddresses: entry.ExtendedAddresses }));
        }
        if (datasets.includes('emails_extended') && entry.ExtendedEmails) {
            const { normalizeContacts } = require('./normalizers/bigdatacorp/basicData');
            Object.assign(normalized, normalizeContacts({ ExtendedEmails: entry.ExtendedEmails }));
        }
        if (datasets.includes('online_presence') && entry.OnlinePresence) {
            const { normalizeOnlinePresence } = require('./normalizers/bigdatacorp/onlinePresence');
            normalized.bigdatacorpOnlinePresence = normalizeOnlinePresence(entry.OnlinePresence);
        }
        if (datasets.includes('financial_data') && entry.FinantialData) {
            normalized.bigdatacorpFinancialData = entry.FinantialData;
        }
        if (datasets.includes('government_debtors') && entry.GovernmentDebtors) {
            const { normalizeGovernmentDebtors } = require('./normalizers/bigdatacorp/governmentDebtors');
            normalized.bigdatacorpGovernmentDebtors = normalizeGovernmentDebtors(entry.GovernmentDebtors);
        }
        if (datasets.includes('collections') && entry.Collections) {
            const { normalizeCollections } = require('./normalizers/bigdatacorp/collections');
            normalized.bigdatacorpCollections = normalizeCollections(entry.Collections);
        }
        if (datasets.includes('relationships') && entry.Relationships) {
            const { normalizeRelationships } = require('./normalizers/bigdatacorp/relationships');
            normalized.bigdatacorpRelationships = normalizeRelationships(entry.Relationships);
        }
        if (datasets.includes('owners_kyc') && entry.OwnersKycData) {
            const { normalizeOwnersKyc } = require('./normalizers/bigdatacorp/ownersKyc');
            normalized.bigdatacorpOwnersKyc = normalizeOwnersKyc(entry.OwnersKycData);
        }
        if (datasets.includes('activity_indicators') && entry.ActivityIndicators) {
            const { normalizeActivityIndicators } = require('./normalizers/bigdatacorp/activityIndicators');
            normalized.bigdatacorpActivityIndicators = normalizeActivityIndicators(entry.ActivityIndicators);
        }
        if (datasets.includes('company_evolution') && entry.CompanyEvolutionData) {
            const { normalizeCompanyEvolution } = require('./normalizers/bigdatacorp/companyEvolution');
            normalized.bigdatacorpCompanyEvolution = normalizeCompanyEvolution(entry.CompanyEvolutionData);
        }
        if (datasets.includes('class_organization') && entry.Memberships) {
            normalized.bigdatacorpMemberships = entry.Memberships;
        }
        if (datasets.includes('historical_basic_data') && entry.HistoricalBasicData) {
            const { normalizeHistoricalBasicData } = require('./normalizers/bigdatacorp/historicalBasicData');
            normalized.bigdatacorpHistoricalBasicData = normalizeHistoricalBasicData(entry.HistoricalBasicData);
        }
        if (datasets.includes('financial_risk') && entry.FinancialRisk) {
            const { normalizeFinancialRisk } = require('./normalizers/bigdatacorp/risk');
            normalized.bigdatacorpFinancialRisk = normalizeFinancialRisk(entry.FinancialRisk);
        }
        if (datasets.includes('indebtedness_question') && entry.IndebtednessQuestion) {
            const { normalizeIndebtedness } = require('./normalizers/bigdatacorp/risk');
            normalized.bigdatacorpIndebtedness = normalizeIndebtedness(entry.IndebtednessQuestion);
        }
        if (datasets.includes('media_profile_and_exposure') && entry.MediaProfileAndExposure) {
            const { normalizeMediaProfile } = require('./normalizers/bigdatacorp/mediaProfile');
            normalized.bigdatacorpMediaProfile = normalizeMediaProfile(entry.MediaProfileAndExposure);
        }
        if (datasets.includes('lawsuits_distribution_data') && entry.LawsuitsDistributionData) {
            const { normalizeLawsuitsDistribution } = require('./normalizers/bigdatacorp/lawsuitsDistribution');
            normalized.bigdatacorpLawsuitsDistribution = normalizeLawsuitsDistribution(entry.LawsuitsDistributionData);
        }
        if (datasets.includes('profession_data') && entry.ProfessionData) {
            Object.assign(normalized, normalizeBigDataCorpProfession(entry.ProfessionData));
        }

        // PJ datasets
        if (datasets.includes('owners_lawsuits') && entry.OwnersLawsuits) {
            const { normalizeOwnersLawsuits } = require('./normalizers/bigdatacorp/ownersLawsuits');
            normalized.bigdatacorpOwnersLawsuits = normalizeOwnersLawsuits(entry.OwnersLawsuits);
        }
        if (datasets.includes('employees_kyc') && entry.EmployeesKycData) {
            const { normalizeEmployeesKyc } = require('./normalizers/bigdatacorp/employeesKyc');
            normalized.bigdatacorpEmployeesKyc = normalizeEmployeesKyc(entry.EmployeesKycData);
        }
        if (datasets.includes('economic_group_kyc') && entry.EconomicGroupKycData) {
            const { normalizeEconomicGroupKyc } = require('./normalizers/bigdatacorp/economicGroupKyc');
            normalized.bigdatacorpEconomicGroupKyc = normalizeEconomicGroupKyc(entry.EconomicGroupKycData);
        }
        if (datasets.includes('dynamic_qsa_data') && entry.DynamicQSAData) {
            const { normalizeDynamicQsa } = require('./normalizers/bigdatacorp/dynamicQsa');
            normalized.bigdatacorpDynamicQsa = normalizeDynamicQsa(entry.DynamicQSAData);
        }
        if (datasets.includes('history_basic_data') && entry.HistoryBasicData) {
            const { normalizeHistoryBasicData } = require('./normalizers/bigdatacorp/historyBasicData');
            normalized.bigdatacorpHistoryBasicData = normalizeHistoryBasicData(entry.HistoryBasicData);
        }
        if (datasets.includes('merchant_category_data') && entry.MerchantCategoryData) {
            const { normalizeMerchantCategory } = require('./normalizers/bigdatacorp/merchantCategory');
            normalized.bigdatacorpMerchantCategory = normalizeMerchantCategory(entry.MerchantCategoryData);
        }
        if (datasets.includes('syndicate_agreements') && entry.SyndicateAgreements) {
            const { normalizeSyndicateAgreements } = require('./normalizers/bigdatacorp/syndicateAgreements');
            normalized.bigdatacorpSyndicateAgreements = normalizeSyndicateAgreements(entry.SyndicateAgreements);
        }
        if (datasets.includes('social_conscience') && entry.SocialConsciousness) {
            const { normalizeSocialConscience } = require('./normalizers/bigdatacorp/socialConscience');
            normalized.bigdatacorpSocialConscience = normalizeSocialConscience(entry.SocialConsciousness);
        }
        if (datasets.includes('online_ads') && entry.OnlineAds) {
            const { normalizeOnlineAds } = require('./normalizers/bigdatacorp/onlineAds');
            normalized.bigdatacorpOnlineAds = normalizeOnlineAds(entry.OnlineAds);
        }

        // Strip _source for preview (internal metadata)
        delete normalized._source;

        await writeAuditEvent({
            action: 'provider.preview_executed',
            tenantId: targetTenantId,
            actor: { type: ACTOR_TYPE.ANALYST, id: uid, email: profile.email || uid },
            entity: { type: 'PROVIDER_PREVIEW', id: doc, label: `Preview BDC ${datasets.join(',')}` },
            source: SOURCE.ANALYST_PORTAL,
            ip: getClientIp(request),
            metadata: { provider: 'bigdatacorp', datasets, doc, subjectType },
        }).catch(() => {});
...
exports.v2MarkProductIntroSeen = onCall(
    { region: 'southamerica-east1', timeoutSeconds: 30 },
    async (request) => {
        const uid = request.auth?.uid;
        if (!uid) throw new HttpsError('unauthenticated', 'Autenticacao necessaria.');

        const { productKey } = request.data || {};
        if (!productKey || typeof productKey !== 'string') {
            throw new HttpsError('invalid-argument', 'productKey e obrigatorio.');
        }

        // Validate against PRODUCT_REGISTRY and sanitize path
        const safeKey = productKey.replace(/[^a-zA-Z0-9_-]/g, '');
        if (safeKey !== productKey) {
            throw new HttpsError('invalid-argument', 'productKey contem caracteres invalidos.');
        }
        if (!PRODUCT_REGISTRY[safeKey]) {
            throw new HttpsError('invalid-argument', `productKey '${safeKey}' nao existe no catalogo.`);
        }

        const profile = await getClientUserProfile(uid);
        const introRef = db.collection('userProductIntros').doc(`${uid}_${safeKey}`);
        await introRef.set({
            uid,
        ...
        return { success: true, productKey: safeKey };
    },
);
...
exports.createClientSolicitation = onCall(
    ...
    async (request) => {
        ...
        const {
            ...
            tagIds = [],
            parameters = {},
            autoMarkRelevant = false,
            autoProcess = false,
        } = request.data || {};

        const safeSubjectKind = ['pf', 'pj', 'hybrid'].includes(subjectKind) ? subjectKind : 'pf';

        let candidateName;
        let taxIdDigits;
        let taxIdMasked;

        if (safeSubjectKind === 'pj') {
            candidateName = String(legalName || '').trim();
            taxIdDigits = sanitizeCnpj(cnpj);
            taxIdMasked = maskCnpj(taxIdDigits);
            if (!candidateName || taxIdDigits.length !== 14) {
                throw new HttpsError('invalid-argument', 'Razao social e CNPJ valido sao obrigatorios.');
            }
        } else {
            candidateName = String(fullName || '').trim();
            taxIdDigits = sanitizeCpf(cpf);
            taxIdMasked = maskCpf(taxIdDigits);
            if (!candidateName || taxIdDigits.length !== 11) {
                throw new HttpsError('invalid-argument', 'Nome completo e CPF valido sao obrigatorios.');
            }
        }

        // Resolve dossier configuration via schema engine
        const dossierConfig = resolveDossierConfiguration({
            productKey: productKey || request.data?.productKey || 'dossier_pf_basic',
            dossierPresetKey: dossierPresetKey || null,
            dossierSchemaKey: dossierSchemaKey || null,
            subjectType: safeSubjectKind,
            requestedModuleKeys: request.data?.requestedModuleKeys || [],
            requestedSectionKeys: request.data?.requestedSectionKeys || [],
            requestedMacroAreaKeys: request.data?.requestedMacroAreaKeys || [],
        });

        if (dossierPresetKey && !dossierConfig.preset) {
            throw new HttpsError('invalid-argument', `Preset de dossiê '${dossierPresetKey}' não existe.`);
        }
        if (dossierSchemaKey && !dossierConfig.schema) {
            throw new HttpsError('invalid-argument', `Schema de dossiê '${dossierSchemaKey}' não existe.`);
        }
        if (dossierConfig.preset && !dossierConfig.preset.subjectTypes.includes(safeSubjectKind)) {
            throw new HttpsError('invalid-argument', `Preset '${dossierPresetKey}' não suporta subjectKind '${safeSubjectKind}'.`);
        }

        const configurationHash = require('crypto')
            .createHash('sha256')
            .update(JSON.stringify({
                presetKey: dossierConfig.preset?.presetKey || null,
                schemaKey: dossierConfig.schema?.schemaKey || null,
                subjectKind: safeSubjectKind,
                requestedModuleKeys: dossierConfig.requestedModuleKeys || [],
                requestedSectionKeys: dossierConfig.requestedSectionKeys || [],
                requestedMacroAreaKeys: dossierConfig.requestedMacroAreaKeys || [],
                parameters,
                autoMarkRelevant,
                autoProcess,
            }))
        ...
        batch.set(candidateRef, {
            ...
            candidateName,
            ...(safeSubjectKind === 'pj'
                ? { cnpj: taxIdDigits, cnpjMasked: taxIdMasked }
                : { cpf: taxIdDigits, cpfMasked: taxIdMasked }),
        ...
        const productKey = (request.data?.productKey || (safeSubjectKind === 'pj' ? 'dossier_pj' : 'dossier_pf_basic')).replace(/[^a-zA-Z0-9_-]/g, '');
        const productContract = getProductContract(productKey);
        if (!productContract) {
            throw new HttpsError('invalid-argument', `Produto '${productKey}' nao existe no catalogo.`);
        }
        ...
        // Validate requested modules against product contract
        ...
        if (invalidModules.length > 0) {
            throw new HttpsError(
                'invalid-argument',
                `Modulos nao contratados: ${invalidModules.join(', ')}`
            );
        }
        ...
        const casePayload = {
            ...
            productKey,
            dossierPresetKey: dossierPresetKey || null,
            dossierSchemaKey: dossierConfig.schema?.schemaKey || null,
            subjectKind: safeSubjectKind,
            tagIds: Array.isArray(tagIds) ? tagIds : [],
            parameters: normalizePlainObject(parameters),
            autoMarkRelevant: Boolean(autoMarkRelevant),
            autoProcess: Boolean(autoProcess),
            configurationHash,
            moduleFreshnessPolicies: requestedModuleKeys.reduce((acc, mk) => {
                acc[mk] = getFreshnessPolicy(mk);
                return acc;
            }, {}),
            requestedModuleKeys,
...
exports.v2GetFeatureFlags = onCall(
    { region: 'southamerica-east1', timeoutSeconds: 30 },
    async (request) => {
        const uid = request.auth?.uid;
        if (!uid) throw new HttpsError('unauthenticated', 'Autenticacao necessaria.');
        ...
        return {
            tenantId,
            flags: resolved.featureFlags || {},
            modules: resolved.enabledModules || [],
            tier: resolved.tier || 'basic',
        };
    },
);
...
if (process.env.FUNCTIONS_EMULATOR === 'true') {
    exports.__test = {
        v2PreviewBigDataCorp: exports.v2PreviewBigDataCorp,
