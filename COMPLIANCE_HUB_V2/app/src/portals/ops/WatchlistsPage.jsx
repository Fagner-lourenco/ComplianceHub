    ...
    if (!tenantOverride) {
        return (
            <div className="watchlists-page">
                <PageHeader
                    eyebrow="Monitoramento Premium"
                    title="Watchlists"
                    subtitle="Selecione um tenant no topbar para ver as watchlists ativas."
                />
                <EmptyState
                    title="Selecione um tenant"
                    message="O monitoramento continuo opera por tenant — escolha um cliente no seletor acima."
                />
            </div>
        );
    }
    ...
    return (
        <div className="watchlists-page">
            <PageHeader
                eyebrow="Monitoramento Premium"
                title="Watchlists"
                subtitle="Sujeitos em vigilancia continua. O pipeline roda periodicamente, diffa riskSignals e gera alertas."
                metrics={[
                    { label: 'Ativas', value: activeCount, testId: 'watchlists-active-count' },
                    { label: 'Com alertas', value: alertsCount },
                    { label: 'Total', value: watchlists.length },
                ]}
            />
            ...
            {error && (
                <EmptyState variant="error" title="Falha ao carregar" message={error} />
            )}

            {loading ? (
                <EmptyState variant="loading" title="Carregando watchlists" message="Sincronizando com o monitor premium..." />
            ) : filtered.length === 0 ? (
                <EmptyState
                    testId="watchlists-empty"
                    title="Nenhuma watchlist neste filtro"
                    message="Crie watchlists a partir de um caso concluido para comecar a monitorar um sujeito continuamente."
                />
            ) : (
                        ...
                        <li key={w.id} className="watchlists-item" data-testid={`watchlist-${w.id}`}>
                            <div className="watchlists-item__head">
                                ...
                                <span className="watchlists-item__modules">
                                    Modulos: {(w.modules || []).map(getModuleLabel).join(', ') || '—'}
                                </span>
