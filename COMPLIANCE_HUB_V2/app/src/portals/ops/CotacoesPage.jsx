...
import { getProductLabel } from '../../core/productLabels';
...
export default function CotacoesPage() {
    ...
    return (
        <div className="cotacoes-page">
            <PageHeader
                eyebrow="Vendas"
                title="Cotacoes de Produtos"
                subtitle="Solicitacoes de produtos vindas do portal cliente. Aprovar injeta o produto em tenantEntitlements."
                metrics={[
                    { label: 'Pendentes', value: pendingCount, testId: 'cotacoes-pending-count' },
            ...
            {loading ? (
                <EmptyState variant="loading" title="Carregando cotacoes" message="Buscando pedidos pendentes do portal cliente..." />
            ) : filtered.length === 0 ? (
                <EmptyState testId="cotacoes-empty" title="Nenhuma cotacao neste filtro" message="Quando um cliente solicitar uma proposta via catalogo, o pedido aparece aqui." />
            ) : (
                        ...
                        return (
                            <li key={quote.id} className="cotacoes-item" data-testid={`cotacao-${quote.id}`}>
                                <div className="cotacoes-item__head">
                                    ...
                                    <strong className="cotacoes-item__product">{getProductLabel(quote.productKey)}</strong>
